...
**2026-09-19 續查（比不一致更嚴重）**：那個模型**本機根本不存在**——`~/models` 與
HF 快取都沒有，實打 `/v1/chat/completions` 回 **404**。也就是說這半個月「本機推理」
不是慢或品質差，是整條不可用；而 `mlx selftest` 一路 rc=0，因為它只列模型不做推理。
端點活著 ≠ 這個模型叫得動，兩者要分開驗（已補 `mlx selftest --servable`，花一個 token）。
使用者決定：記憶體不足，**暫不部署**本機 MLX，待較小模型再載入。在那之前，憲章
§5b 的「本地 Qwen temperature=0 複審」這項確定性回饋來源不可用，用到就標 UNVERIFIED。
